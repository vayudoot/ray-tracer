...
def right_sphere():
    s = Sphere()
    s.transform = Translation(1.5, 0.5, -0.5) * Scaling(0.5, 0.5, 0.5)
    s.material = Material(c=Color(0.5, 1, 0.1), d=0.7, sp=0.3)
    return s

def left_sphere():
    s = Sphere()
    s.transform = Translation(-1.5, 0.33, -0.75) * Scaling(0.33, 0.33, 0.33)
...
if __name__ == "__main__":
    ...
    c = Camera(500, 250, math.pi/3)
    c.transform = view_transform(Point(0, 1.5, -5), Point(0, 1, 0), Vector(0, 1, 0))
    canvas = c.render(w)

    with open("images/scene.ppm", "w") as ppm_file:
